    ...
    <div class="row mt-3">
        <div class="col-sm-6 offset-sm-3">
            @switch($type)
                @case(\App\Http\Controllers\Misc\Enums\CharacterType::ALT)
                <div class="card card-body border-0 shadow-sm pb-2">
                    <p class="text-center py-4">You can only have one main character. <a href="{{route('alts.delete', ['mainId' => \App\Http\Controllers\Auth\AuthController::getLoginId(), 'altId' => $main->id])}}" class="text-danger">Delete {{$main->name}}</a> your main to add a new one.</p>
                </div>
                @break
                @case(\App\Http\Controllers\Misc\Enums\CharacterType::MAIN)
                <div class="card card-body border-0 shadow-sm pb-2">
