            ...
            @if(\App\Http\Controllers\Auth\AuthController::isLoggedIn())
                <li class="nav-item dropdown" style="list-style: none">
                    <a class="nav-link dropdown-toggle" href="#" id="navbarDropdown" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                        <img src="https://images.evetech.net/characters/{{\App\Http\Controllers\Auth\AuthController::getLoginId()}}/portrait?size=32" alt="{{session()->get('login_name')}}" class="rounded-circle shadow-sm" style="border:1px solid #fff;"> {{session()->get('login_name')}}</a>
                    <div class="dropdown-menu fade-down shadow" aria-labelledby="navbarDropdown">
                       <a class="dropdown-item pl-2 {{"new" == $currentRoute ? "active text-dark" : ""}}" href="{{route("new")}}" ><img src="https://img.icons8.com/small/24/{{App\Http\Controllers\ThemeController::getThemedNavBarIconColor("new" == $currentRoute)}}/new-by-copy.png" class="tinyicon mr-1"/>Add run</a>
                        ...
                        @foreach(\App\Http\Controllers\Profile\AltRelationController::getAllMyAvailableCharacters() as $char)
                            <a class="dropdown-item pl-2" href="{{route('alts.switch', ['altId' => $char->name])}}">{{$char->name}}<a>
                        @endforeach
                        <div class="dropdown-divider"></div>
